...
| Find a user ID to assign a task           | `GET /v2/team` — IDs ride along in `members[]`         | [HIERARCHY.md#finding-a-user-id](reference/HIERARCHY.md#finding-a-user-id)                                                                                         |
...
| Read back shows `�` where a letter was    | Flaky read, not lost data — re-read until clean        | [GOTCHAS.md#markdown_description-mangles-a-character-at-random-on-read](reference/GOTCHAS.md#markdown_description-mangles-a-character-at-random-on-read)           |
| Bold vanished from a table cell           | Emphasis is stripped inside cells; keep it in prose    | [GOTCHAS.md#table-cells-drop-bold-and-italic](reference/GOTCHAS.md#table-cells-drop-bold-and-italic)                                                               |
...
| Un-nest a subtask                         | Not possible via API — UI only                         | [GOTCHAS.md#a-subtask-cannot-be-converted-back-to-a-task](reference/GOTCHAS.md#a-subtask-cannot-be-converted-back-to-a-task)                                       |
...
| Field write rejected as `FIELD_033`       | Plan quota on field uses — clearing refunds none       | [GOTCHAS.md#custom-field-writes-are-capped-by-plan](reference/GOTCHAS.md#custom-field-writes-are-capped-by-plan)                                                   |
