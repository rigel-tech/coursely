...
describe('createPaymentRecordedEmailTemplate', () => {
  it('embeds amount, method, and course title', () => {
    const { subject, html, text } = createPaymentRecordedEmailTemplate({
      ...
      paymentDate: '2026-09-19T00:00:00.000Z',
...
describe('escapeHtml', () => {
  it('escapes every character that could open a tag or an entity', () => {
    expect(escapeHtml('<b>Tom & Jerry</b>')).toBe('&lt;b&gt;Tom &amp; Jerry&lt;/b&gt;')
  })
})

describe('class schedule emails', () => {
  it.each([
    ['createClassAssignedEmailTemplate', createClassAssignedEmailTemplate],
    ['createClassRescheduledEmailTemplate', createClassRescheduledEmailTemplate],
  ] as const)('%s escapes student- and staff-entered text in its HTML only', (_name, render) => {
    const { html, text } = render({
      studentNameOrEmail: '<i>Nguyễn</i> & Co',
      courseTitle: 'C++ <Pro>',
      classCode: 'CPP-01',
      startDate: '2026-10-01T00:00:00.000Z',
      scheduleTime: 'Tối <2-4-6>',
    ...
    expect(html).toContain('&lt;i&gt;Nguyễn&lt;/i&gt; &amp; Co')
    expect(html).toContain('C++ &lt;Pro&gt;')
    expect(html).toContain('Tối &lt;2-4-6&gt;')
    expect(html).toContain('Phòng &amp; 101')
    expect(html).not.toContain('<i>Nguyễn</i>')
    expect(text).toContain('<i>Nguyễn</i> & Co')
  })
})
