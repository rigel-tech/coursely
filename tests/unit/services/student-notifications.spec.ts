...
describe('listAndMarkRecentNotifications', () => {
  it('scopes the query to this student, most recent first, limited to 20', async () => {
  ...
  it('marks exactly the returned batch as read — not every unread notification', async () => {
    const find = vi.fn().mockResolvedValue({ docs: [{ id: 101 }, { id: 102 }] })
    const update = vi.fn()
    vi.mocked(getPayload).mockResolvedValue({ find, update } as never)
  ...
  it('returns the fetched documents', async () => {
    const docs = [{ id: 101, title: 'A' }]
    const find = vi.fn().mockResolvedValue({ docs })
    const update = vi.fn()
    vi.mocked(getPayload).mockResolvedValue({ find, update } as never)

    await expect(listAndMarkRecentNotifications(7)).resolves.toBe(docs)
  })
})
